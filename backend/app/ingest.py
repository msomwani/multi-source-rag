...
class PDFIngestor:
    """OOP PDF ingestor: reads PDF, extracts per-page text, attaches metadata, splits into chunks."""
    ...
    def _pages_to_docs(self, pages: List[str]) -> List[Document]:
        ...
        for i, text in enumerate(pages, start=1):
            meta = {"source": str(self.path.name), "type": "pdf", "page": i}
    ...
    def ingest(self) -> List[Document]:
        ...
        for d in page_docs:
            chunked.extend(split_text_to_docs(d.page_content, metadata=d.metadata))
        return chunked